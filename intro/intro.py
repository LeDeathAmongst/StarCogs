...
class Intro(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        default_guild = {'fields': [], 'intro_channel': None, 'title':
            'Introduction', 'footer': None, 'break_field_title': None}
        default_user = {'color': None, 'fields': {},
    ...
    async def intro_addvalue(self, ctx, field_name: str, *, field_value: str):
        ...
        await ctx.send(
            f'Field `{field_name}` has been set to `{field_value}` in your introduction.'
            )
    ...
    async def intro_removevalue(self, ctx, field_name: str):
        ...
        if field_name in fields:
            ...
            await self.config.user(ctx.author).fields.set(fields)
            await ctx.send(
                f'Field `{field_name}` has been removed from your introduction.'
                )
        else:
            await ctx.send(
                f'Field `{field_name}` does not exist in your introduction.')
    ...
    async def intro_editvalue(self, ctx, field_name: str, *, field_value: str):
        ...
        if field_name in fields:
            ...
            await self.config.user(ctx.author).fields.set(fields)
            await ctx.send(
                f'Field `{field_name}` has been updated to `{field_value}` in your introduction.'
                )
        else:
            await ctx.send(
                f'Field `{field_name}` does not exist in your introduction. Use `intro addvalue` to add it first.'
                )
    ...
    async def intro_viewfields(self, ctx):
        ...
        if not fields:
            await ctx.send(
                'No fields have been added to the introduction form for this server.'
                )
            return
        embed = discord.Embed(title='Available Fields for Introduction',
            description=
            'The following fields are available for your introduction in this server:'
            , color=discord.Color.blue())
        for field in fields:
            embed.add_field(name=field.capitalize(), value=
                f'Use `[p]intro addvalue {field} <value>` to set this field.',
                inline=False)
    ...
    async def intro_preview(self, ctx):
        ...
        break_field_title = await self.config.guild(ctx.guild
            ).break_field_title()
        if not color or not fields:
            await ctx.send(
                'You need to set your introduction color and fields first.')
            return
        embed = discord.Embed(color=color, title=title)
        for field_name, field_value in fields.items():
            embed.add_field(name=field_name.capitalize(), value=field_value,
                inline=False)
        if break_field_title:
            embed.add_field(name=break_field_title, value='\u200b', inline=
                False)
        if footer:
    ...
    async def intro_send(self, ctx):
        ...
        if intro_channel_id:
            ...
            if intro_channel:
                last_intro_message_id = await self.config.user(ctx.author
                    ).last_intro_message_id()
                if last_intro_message_id:
                    try:
                        last_intro_message = await intro_channel.fetch_message(
                            last_intro_message_id)
                        await last_intro_message.delete()
                    except discord.NotFound:
                        pass
                new_intro_message = await intro_channel.send(
                    f'{ctx.author.mention}', embed=embed)
                await self.config.user(ctx.author).last_intro_message_id.set(
                    new_intro_message.id)
                await ctx.send('Your introduction has been sent!')
            else:
                await ctx.send(
                    'The configured introduction channel does not exist.')
        else:
            await ctx.send(
                'The introduction channel has not been set by the server owner or admin.'
                )
    ...
    async def intro_setchannel(self, ctx, channel: discord.TextChannel):
        ...
        await ctx.send(
            f'The introduction channel has been set to {channel.mention}')
    ...
    async def intro_addfield(self, ctx, field_name: str):
        ...
        if field_name not in fields:
            ...
            await ctx.send(
                f'The field `{field_name}` has been added to the introduction form.'
                )
        else:
            await ctx.send(
                f'The field `{field_name}` already exists in the introduction form.'
                )
    ...
    async def intro_removefield(self, ctx, field_name: str):
        ...
        if field_name in fields:
            ...
            await self.config.guild(ctx.guild).fields.set(fields)
            await ctx.send(
                f'The field `{field_name}` has been removed from the introduction form.'
                )
        else:
            await ctx.send(
                f'The field `{field_name}` does not exist in the introduction form.'
                )
    ...
    async def intro_setbreakfield(self, ctx, *, break_field_title: str):
        ...
        await self.config.guild(ctx.guild).break_field_title.set(
            break_field_title)
        await ctx.send(
            f'The break field title has been set to `{break_field_title}`.')
    ...
    async def intro_example(self, ctx):
        ...
        await self.config.guild(ctx.guild).break_field_title.set(
            example_break_field_title)
        await ctx.send(
            'Example introduction has been set with predefined fields, title, footer, and break field title.'
            )
