...
class AdvancedInviteTracker(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        default_guild = {'invites': {}, 'invite_counts': {}, 'log_channel':
            None, 'variables': {}}
    ...
    async def initialize(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                print(
                    f'Missing permissions to fetch invites for guild: {guild.name} ({guild.id})'
                    )
    ...
    async def on_member_join(self, member):
        ...
        for invite in invites_before:
            if invite.uses < next((i.uses for i in invites_after if i.code ==
        ...
        if invite_used:
            ...
            async with self.config.guild(member.guild).invite_counts(
                ) as invite_counts:
    ...
    async def send_invite_embed(self, member, inviter, action):
        if not inviter:
            return
        embed = discord.Embed(title=f'User {action.capitalize()}',
            description=f'{member.mention} has {action} the server.', color
            =discord.Color.green() if action == 'joined' else discord.Color
            .red(), timestamp=datetime.utcnow())
    ...
    async def view_invites(self, ctx, member: discord.Member=None):
        ...
        if inviter_id in invites:
            embed = discord.Embed(title=
                f'Invites for {member.display_name}', color=discord.Color.
                blue())
            embed.add_field(name='Total Invites', value=len(invites[
                inviter_id]))
            embed.add_field(name='Joined', value=invite_counts.get(
                inviter_id, {}).get('joined', 0))
            embed.add_field(name='Left', value=invite_counts.get(inviter_id,
                {}).get('left', 0))
    ...
    @commands.admin_or_permissions(manage_guild=True)
    async def delete_invite(self, ctx, inviter: discord.Member, invite_code:
        str):
        ...
        async with self.config.guild(ctx.guild).invites() as invites:
            ...
            if inviter_id in invites and invite_code in invites[inviter_id]:
                ...
                await ctx.send(
                    f'Invite `{invite_code}` removed for {inviter.mention}.')
            else:
                await ctx.send(
                    f'{inviter.mention} does not have invite `{invite_code}` tracked.'
                    )
    ...
    async def invite_leaderboard(self, ctx):
        ...
        invite_counts = await self.config.guild(ctx.guild).invite_counts()
        sorted_invites = sorted(invite_counts.items(), key=lambda x: x[1][
            'joined'] - x[1]['left'], reverse=True)
        pages = []
        page = []
        for i, (inviter_id, counts) in enumerate(sorted_invites, 1):
            ...
            net = joined - left
            page.append(
                f'{i}. {inviter.mention} - Invites: {net} (Joined: {joined}, Left: {left})'
                )
            if len(page) == 10:
                pages.append('\n'.join(page))
                page = []
        ...
        for i, page in enumerate(pages, 1):
            embed = discord.Embed(title=
                f'Invite Leaderboard - Page {i}/{len(pages)}', description=
                page, color=discord.Color.blue())
            await ctx.send(embed=embed)
    ...
    async def role_info(self, ctx, role: discord.Role):
        ...
        embed = discord.Embed(title=f'Role Info - {role.name}', color=role.
            color, timestamp=datetime.utcnow())
        embed.add_field(name='ID', value=role.id)
        embed.add_field(name='Color', value=str(role.color))
        embed.add_field(name='Position', value=role.position)
        embed.add_field(name='Mentionable', value=role.mentionable)
        embed.add_field(name='Permissions', value=', '.join(perm[0] for
            perm in role.permissions if perm[1]))
        await ctx.send(embed=embed)
    ...
    async def server_info(self, ctx):
        ...
        embed = discord.Embed(title=f'Server Info - {guild.name}',
            description=guild.description or 'No description', color=
            discord.Color.blue(), timestamp=datetime.utcnow())
    ...
    async def stats(self, ctx):
        ...
        total_invites = sum(counts['joined'] for counts in invite_counts.
            values())
        total_left = sum(counts['left'] for counts in invite_counts.values())
        net_invites = total_invites - total_left
        embed = discord.Embed(title=f'Server Stats - {guild.name}', color=
            discord.Color.blue(), timestamp=datetime.utcnow())
    ...
    async def support(self, ctx):
        ...
        embed = discord.Embed(title='Invite Tracker Support', description=
            'For support, please visit our [support server](https://discord.gg/DmGBCrRtQz).'
    ...
    async def user_info(self, ctx, member: discord.Member):
        ...
        embed.add_field(name='Created At', value=member.created_at.strftime
            ('%Y-%m-%d %H:%M:%S'))
        embed.add_field(name='Roles', value=', '.join(role.mention for role in
            member.roles if role != ctx.guild.default_role))
        await ctx.send(embed=embed)
    ...
    async def variables(self, ctx, action: str, key: str=None, value: str=None
        ...
        if action == 'view':
            ...
            if key:
                ...
                embed = discord.Embed(title='Invite Tracker Variables',
            ...
            await ctx.send(
                'Invalid action or missing key/value. Use `view`, `set <key> <value>`, or `delete <key>`.'
                )
    ...
    async def on_invite_create(self, invite):
        try:
            ...
            print(
                f'Missing permissions to fetch invites for guild: {invite.guild.name} ({invite.guild.id})'
                )
