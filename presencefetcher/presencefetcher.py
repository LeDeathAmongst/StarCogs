...
class PresenceFetcher(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        default_guild = {'presence_changes': {}}
    ...
    async def shared(self, ctx, user: discord.User):
        ...
        shared_guilds = [guild for guild in self.bot.guilds if guild.
            get_member(user.id)]
        embed = discord.Embed(title=
            f'Status of {user.display_name} across shared servers', color=
            discord.Color.blue())
        if shared_guilds:
            for guild in shared_guilds:
                member = guild.get_member(user.id)
                status_type = self.get_status_type(member.status)
                custom_status = next((activity for activity in member.
                    activities if isinstance(activity, discord.
                    CustomActivity)), None)
                status_text = status_type
                if custom_status:
                    status_text += f'\nCustom Status: {custom_status.name}'
                embed.add_field(name=guild.name, value=status_text, inline=
                    False)
        else:
    ...
    async def past(self, ctx, user: discord.User):
        ...
        embed = discord.Embed(title=f'Past Statuses of {user.display_name}',
        ...
        if changes:
            for change in changes:
                ...
                embed.add_field(name=timestamp.strftime('%Y-%m-%d %H:%M:%S'
                    ), value=
                    f"""Status: {status}
Custom Status: {custom_status}""",
    ...
    async def allusers(self, ctx):
        ...
        embed = discord.Embed(title=
            f'Statuses of All Users in {ctx.guild.name}', color=discord.
            Color.green())
        for member in members:
            status_type = self.get_status_type(member.status)
            custom_status = next((activity for activity in member.
                activities if isinstance(activity, discord.CustomActivity)),
                None)
            status_text = status_type
            if custom_status:
                status_text += f'\nCustom Status: {custom_status.name}'
            embed.add_field(name=member.display_name, value=status_text,
                inline=True)
        await ctx.send(embed=embed)
    ...
    async def on_presence_update(self, before: discord.Member, after:
        discord.Member):
        if (before.status != after.status or before.activities != after.
            activities):
            guild = after.guild
            if guild:
                async with self.config.guild(guild).presence_changes(
                    ) as presence_changes:
                    changes = presence_changes.get(str(after.id), [])
                    custom_status = next((activity for activity in after.
                        activities if isinstance(activity, discord.
                        CustomActivity)), None)
                    changes.append({'timestamp': datetime.utcnow().
                        isoformat(), 'status': self.get_status_type(after.
                        status), 'custom_status': custom_status.name if
                        custom_status else 'No custom status'})
                    changes = [change for change in changes if datetime.
                        fromisoformat(change['timestamp']) > datetime.
