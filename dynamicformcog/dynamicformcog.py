...
class DynamicFormCog(Cog):

    def __init__(self, bot):
        self.bot = bot
        self.config = Config.get_conf(self, identifier=1234567890)
        default_global = {'api_key': None}
        default_guild = {'questions': {}, 'form_id': None}
    ...
    async def add_question(self, ctx, *, question_text):
        ...
        async with self.config.guild(ctx.guild).questions() as questions:
            questions[question_id] = {'type': 'short_text', 'title':
                question_text}
    ...
    async def edit_question(self, ctx, question_id: str, *, new_question_text):
        ...
        async with self.config.guild(ctx.guild).questions() as questions:
            if question_id in questions:
                ...
                await ctx.send(
                    f'Edited question {question_id}: {new_question_text}')
            else:
    ...
    async def remove_question(self, ctx, question_id: str):
        ...
        async with self.config.guild(ctx.guild).questions() as questions:
            if question_id in questions:
                ...
                await ctx.send(
                    f"Removed question {question_id}: {removed_question['title']}"
                    )
            else:
                await ctx.send(f'Invalid question ID: {question_id}')
    ...
    async def create_form(self, ctx, *, form_title):
        ...
        if not api_key:
            await ctx.send(
                'API key not set. Use `!formconfig setapikey <api_key>` to set it.'
                )
            return
        questions = await self.config.guild(ctx.guild).questions()
        if not questions:
            await ctx.send(
                'No questions added. Use `!addquestion` to add questions.')
            return
        headers = {'Authorization': f'Bearer {api_key}', 'Content-Type':
            'application/json'}
        form_data = {'title': form_title, 'fields': list(questions.values())}
        response = requests.post('https://api.typeform.com/forms', headers=
        ...
        if response.status_code == 201:
            ...
            await ctx.send(
                f'Failed to create form: {response.status_code} {response.text}'
                )
    ...
    async def get_responses(self, ctx):
        ...
        if not form_id:
            await ctx.send(
                'No form created. Use `!createform <title>` to create a form.')
            return
        headers = {'Authorization': f'Bearer {api_key}'}
        response = requests.get(
            f'https://api.typeform.com/forms/{form_id}/responses', headers=
            headers)
        if response.status_code == 200:
            ...
            for item in data['items']:
                ...
                for answer in answers:
                    response_message += (
                        f"{answer['field']['id']}: {answer.get('text', 'N/A')}\n"
                        )
            await ctx.send(response_message)
        else:
            await ctx.send(
                f'Failed to get responses: {response.status_code} {response.text}'
                )
