...
class GoogleSearch(Cog):
    """Google Search cog"""

    def __init__(self, bot):
        self.bot = bot
        self.config = Config.get_conf(self, identifier=1234567890)
        default_global = {'api_key': '', 'cse_id': ''}
    ...
    async def google(self, ctx, *, query: str):
        ...
        if not api_key or not cse_id:
            await ctx.send(
                'API key or Custom Search Engine ID is not set. Use `!setapikey` and `!setcseid` to set them.'
                )
            return
        search_url = (
                ...
                if 'items' in data:
                    ...
                    async with session.get(link) as site_response:
                        if site_response.status != 200:
                            await ctx.send(
                                f'Failed to fetch content from {link}.')
                            return
