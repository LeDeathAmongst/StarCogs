...
class RestartMsg(Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.config = Config.get_conf(self, identifier='restartmsg',
    ...
    async def warnrestart(self, ctx, *, message: str):
        ...
        embed = discord.Embed(title='Incoming Restart', description=message,
        ...
        await ctx.send(embed=discord.Embed(title='Restarting', description=
            'Restart message sent to all guild owners. Restarting bot...',
    ...
    async def testmsg(self, ctx, guild_id: int, *, message: str):
        ...
        embed = discord.Embed(title='Testing a Restart Warning',
            ...
            if owner:
                try:
                    ...
                    await ctx.send(embed=discord.Embed(title='Message Sent',
                        description=
                        f'Test message sent to the owner of `{guild.name}`.',
                        color=discord.Color.green()))
                except Exception as e:
                    await ctx.send(embed=discord.Embed(title='ErRoR 404',
                        description=
                        f'Failed to send message to {owner}: {e}', color=
                        discord.Color.red()))
        else:
            await ctx.send(embed=discord.Embed(title='ErRoR 404',
                description=f'Guild with ID {guild_id} not found.', color=
                discord.Color.red()))
    ...
    async def ignoreowner(self, ctx, guild_id: int):
        ...
        if guild:
            ...
            if guild_id not in ignore_servers:
                ...
                embed = discord.Embed(title='Now Ignoring', color=discord.
                    Color.orange())
                embed.add_field(name='Server Name', value=guild.name,
                    inline=False)
                embed.add_field(name='Server ID', value=guild.id, inline=False)
                embed.add_field(name='Server Owner Name', value=owner.name,
                    inline=False)
                ...
                await ctx.send(embed=discord.Embed(title='ErRoR 404',
                    description=
                    f'Owner of server with ID {guild_id} is already ignored.',
                    color=discord.Color.red()))
        else:
            await ctx.send(embed=discord.Embed(title='ErRoR 404',
                description=f'Guild with ID {guild_id} not found.', color=
    ...
    async def unignoreowner(self, ctx, guild_id: int):
        ...
        if guild:
            ...
            if guild_id in ignore_servers:
                ...
                await self.config.ignore_servers.set(ignore_servers)
                embed = discord.Embed(title='No Longer Ignoring', color=
                    discord.Color.green())
                embed.add_field(name='Server Name', value=guild.name,
                    inline=False)
                embed.add_field(name='Server ID', value=guild.id, inline=False)
                ...
                await ctx.send(embed=discord.Embed(title='ErRoR 404',
                    description=
                    f'Owner of server with ID {guild_id} is not in the ignore list.'
                    , color=discord.Color.red()))
        else:
            await ctx.send(embed=discord.Embed(title='ErRoR 404',
                description=f'Guild with ID {guild_id} not found.', color=
