...
class ServerInfo(Cog):
    ...
    async def serverinfo(self, ctx, invite_link: str):
        ...
        try:
            ...
            embed = discord.Embed(title=f'Server Info: {guild_name}', color
                =discord.Color.blue())
            if icon_url:
                embed.set_thumbnail(url=icon_url)
            embed.add_field(name='Guild ID', value=guild_id, inline=True)
            embed.add_field(name='Member Count', value=member_count, inline
                =True)
            embed.add_field(name='Online Members', value=online_count,
                inline=True)
            ...
            await ctx.send(
                f'An error occurred while fetching the invite: {str(e)}')
