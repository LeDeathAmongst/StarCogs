...
class FormManager(Cog):
    ...
    async def create(self, ctx, form_name: str):
        ...
        await ctx.send(
            f"Form '{form_name}' created. Use `[p]form addquestion {form_name} <question>` to add questions."
            )
    ...
    async def addquestion(self, ctx, form_name: str, *, question: str):
        ...
        await self.config.guild(ctx.guild).forms.set(forms)
        await ctx.send(
            f"Question added to form '{form_name}' with ID {question_id}.")
    ...
    async def list(self, ctx):
        ...
        forms = await self.config.guild(ctx.guild).forms()
        if forms:
            embed = discord.Embed(title='Available Forms', color=discord.
                Color.blue())
            for form_name, questions in forms.items():
                embed.add_field(name=form_name, value=
                    f'{len(questions)} questions', inline=False)
    ...
    @form.command()
    async def edit(self, ctx, form_name: str, question_id: str, *,
        new_question: str):
        ...
        for question in forms[form_name]:
            if question['id'] == question_id:
                ...
                await self.config.guild(ctx.guild).forms.set(forms)
                await ctx.send(
                    f"Question with ID {question_id} in form '{form_name}' has been updated."
                    )
                return
        await ctx.send(
            f"No question found with ID {question_id} in form '{form_name}'.")
        ...
        for question in questions:
            ...
            try:
                response = await self.bot.wait_for('message', check=check,
                    timeout=300)
                responses.append({'id': question['id'], 'response':
                    response.content})
    ...
    async def viewresponses(self, ctx, user: discord.Member, form_name: str):
        ...
        if ctx.guild.id in responses and form_name in responses[ctx.guild.id]:
            embed = discord.Embed(title=f"Responses for '{form_name}'",
                color=discord.Color.green())
            form_responses = responses[ctx.guild.id][form_name]
            forms = await self.config.guild(ctx.guild).forms()
            questions = {q['id']: q['question'] for q in forms[form_name]}
            for response in form_responses:
                question_text = questions.get(response['id'],
                    'Unknown Question')
                embed.add_field(name=question_text, value=response[
                    'response'], inline=False)
            await ctx.send(embed=embed)
        else:
            await ctx.send(
                f"No responses found for form '{form_name}' from {user.mention}."
                )
