...
class Roast(Cog):
    ...
    def __init__(self, bot):
        ...
        self.predefined_roasts = ["You're as useless as the 'ueue' in 'queue'."
            ,
    ...
    async def roast(self, ctx, user: discord.User=None):
        ...
        if not api_key:
            await ctx.send(
                'OpenAI API key is not set. Please set it using the `setapikey` command.'
                )
            return
        ...
        if roast:
            ...
            await ctx.send(
                "Couldn't generate a roast at the moment. Try again later.")
    ...
    async def generate_roast(self, messages):
        ...
        try:
            response = openai.Completion.create(engine='text-davinci-003',
                prompt=prompt, max_tokens=60, n=1, stop=None, temperature=0.7)
