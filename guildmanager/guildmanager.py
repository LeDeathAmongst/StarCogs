...
class GuildManager(Cog):
    ...
    def __init__(self, bot: Red) ->None:
        ...
        default_global = {'whitelist': [], 'blacklist': [], 'serverlocked':
            False, 'min_members': 0, 'bot_ratio': 0.0, 'log_channel': None}
    ...
    async def whitelist(self, ctx: commands.Context):
        ...
        if not whitelist:
            await ctx.send_help()
            return
        embed = discord.Embed(title=_('Whitelisted Guilds'), description=
            humanize_list([inline(str(n)) for n in whitelist]), color=await
            ctx.embed_color())
    ...
    async def whitelist_add(self, ctx: commands.Context, *guild_ids: int):
        ...
        content = ''
        if success:
            content += _('The following guilds have been whitelisted: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                success)]))
        if failed:
            content += _('\nThe following guilds were already whitelisted: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                failed)]))
    ...
    async def whitelist_remove(self, ctx: commands.Context, *guild_ids: int):
        ...
        if success:
            content += _(
                'The following guilds have been removed from whitelist: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                success)]))
        if failed:
            content += _('\nThe following guilds were in the whitelist: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                failed)]))
        await ctx.send(content=content)
    ...
    async def blacklist(self, ctx: commands.Context):
        ...
        if not blacklist:
            await ctx.send_help()
            return
        embed = discord.Embed(title=_('Blacklisted Guilds'), description=
            humanize_list([inline(str(n)) for n in blacklist]), color=await
            ctx.embed_color())
        await ctx.send(embed=embed)
    ...
    async def blacklist_add(self, ctx: commands.Context, *guild_ids: int):
        ...
        content = ''
        if success:
            content += _('The following guilds have been blacklisted: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                success)]))
        if failed:
            content += _('\nThe following guilds were already blacklisted: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                failed)]))
        await ctx.send(content=content)
    ...
    async def chunk(self, ctx: commands.Context, *guilds: discord.Guild):
        ...
        if failed:
            content += _('\nThe following guilds were already chunked: {}'
                ).format(humanize_list([inline(str(n)) for n in sorted(
                failed)]))
        await ctx.send(content=content)

    @guildmanager.command()
    async def channel(self, ctx: commands.Context, channel: discord.
        TextChannel=None):
    ...
    async def minmembers(self, ctx: commands.Context, min_members: int=0):
        ...
        await ctx.send(_('The minimum member limit has been set to {}.').
            format(min_members) if min_members else _(
            'The minimum member limit has been removed.'))
    ...
    async def botratio(self, ctx: commands.Context, ratio: int=0):
        ...
        await ctx.send(_('The bot ratio has been set to {}%.').format(ratio
            ) if ratio else _('The bot ratio has been removed.'))
    ...
    async def settings(self, ctx: commands.Context):
        ...
        description = [f'`Log Channel     :` {log_chan}',
            f"`Minimum Members :` {config['min_members'] or '-'}",
            f"`Bot Ratio       :` {round(config['bot_ratio'] * 100) or '-'}%"]
        embed = discord.Embed(title=_('Guild Manager Settings'),
            description='\n'.join(description), color=await ctx.embed_color())
        await ctx.send(embed=embed)
    ...
    async def show_botfarms(self, ctx: commands.Context):
        ...
        async for guild in AsyncIter(self.bot.guilds):
            bot_ratio = len([m for m in guild.members if m.bot]
                ) / guild.member_count
        ...
        async for guild in AsyncIter(guilds):
            ...
            value = _('Bot Ratio: {}').format(bold(str(round(bot_ratio * 
                100, 2)) + '%'))
            fields.append({'name': guild_name, 'value': value, 'inline': True})
        embeds = await self.pagify_embed_fields(*fields, title=_(
            'Bot Farms ({}% Bots)').format(round(config['bot_ratio'] * 100)
    ...
    async def show_lessmembers(self, ctx: commands.Context):
        ...
        if not guilds:
            await ctx.send(_(
                'No servers with less members than minimum found.'))
            return
        fields = []
        async for guild in AsyncIter(guilds):
            guild_name = f'{guild.name}\n'
            if guild.id in whitelist:
                guild_name += _('(Whitelisted)')
            value = _('Members: {}').format(bold(humanize_number(guild.
                member_count)))
            fields.append({'name': guild_name, 'value': value, 'inline': True})
        embeds = await self.pagify_embed_fields(*fields, title=_(
            'Servers With Less Than {} Members)').format(config[
            'min_members']), color=discord.Color.red())
        await menu(ctx, embeds)
    ...
    async def unchunked(self, ctx: commands.Context):
        ...
        async for guild in AsyncIter(guilds):
            ...
            value = [_('Members: {}').format(bold(humanize_number(len(guild
                .members)))), _('Cached: {}').format(bold(str(round(cached *
                100, 2)) + '%'))]
            fields.append({'name': guild.name, 'value': '\n'.join(value),
                'inline': True})
        embeds = await self.pagify_embed_fields(*fields, title=_(
            'Unchunked Guilds'), color=discord.Color.red())
        await menu(ctx, embeds)

    @staticmethod
    async def pagify_embed_fields(*fields: Dict[str, Union[str, bool]],
        per_embed: int=9, **kwargs) ->List[discord.Embed]:
    ...
    async def blacklisted(self, ctx: commands.Context):
        ...
        view.message = await ctx.send(_(
            'Do you want me to leave {} blacklisted guilds?').format(len(
            guilds)), view=view)
        await view.wait()
        if view.result:
            await self.leave_guilds(guilds)
            content = _('Done. I have left {} blacklisted guilds.').format(len
                (guilds))
            await view.message.edit(content=content)
        else:
            await view.message.edit(content=_(
                "Ok, I won't leave any blacklisted guilds."))
    ...
    async def leave_botfarms(self, ctx: commands.Context):
        ...
        view = ConfirmView(ctx.author, disable_buttons=True)
        view.message = await ctx.send(_(
            'Do you want me to leave {} bot farms?').format(len(guilds)),
            view=view)
        await view.wait()
        if view.result:
            await self.leave_guilds(guilds)
            ...
            await view.message.edit(content=_(
                "Ok, I won't leave any bot farms."))
    ...
    async def leave_lessmembers(self, ctx: commands.Context):
        ...
        view = ConfirmView(ctx.author, disable_buttons=True)
        view.message = await ctx.send(_(
            'Do you want me to leave {} servers?').format(len(guilds)),
            view=view)
        await view.wait()
        if view.result:
            await self.leave_guilds(guilds)
    ...
    async def get_system_channel(guild: discord.Guild, /):
        ...
        if not (channel and channel.permissions_for(guild.me).send_messages):
            channels = [c for c in guild.text_channels if c.permissions_for
    ...
    async def log_autoleave(self, guild: discord.Guild, title: str, reason: str
        ...
        if not log:
            return
        embed = discord.Embed(title=title, description=reason, color=
    ...
    async def leave_guilds(self, guilds: List[discord.Guild]):
        ...
        async for guild in AsyncIter(guilds):
            ...
            if guild.member_count < min_members:
                self.log_guild_remove = False
                reason = _(
                    "I'm leaving because this server has less than {} members."
                    ).format(min_members)
                if channel:
                    await channel.send(reason)
                await guild.leave()
            ...
            if bot_ratio >= config['bot_ratio']:
                self.log_guild_remove = False
                reason = _(
                    "I'm leaving this server since it has a high bot to member ratio."
                    )
                if channel:
                    await channel.send(reason)
                await guild.leave()
    ...
    async def send_to_log(self, guild: discord.Guild, *, join: bool):
        ...
        description = [f'`Guild      :` {guild.name} ({guild.id})',
            ...
            f'`Humans     :` {humans} Humans', f'`Bots       :` {bots} Bots']
        if join:
            ...
            if inviter:
                description.insert(2,
                    f'`Invited by :` {inviter} ({inviter.id})')
        else:
            title = _('I have left a server!')
        embed = discord.Embed(title=title, description='\n'.join(
            description), color=await self.bot.get_embed_color(guild),
