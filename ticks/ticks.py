...
class Ticks(Cog):
    ...
    async def ticks(self, ctx: commands.Context):
        ...
        embed = discord.Embed(description=
            'Use a subcommand to manage tags or use `ticks run` to execute a tag.'
    ...
    async def add(self, ctx: commands.Context, name: str, *, content: str):
        ...
        async with self.config.guild(ctx.guild).tags() as tags:
            if name in tags:
                embed = discord.Embed(description=
                    f'The tag `{name}` already exists.', color=discord.
                    Color.red())
                await ctx.send(embed=embed)
                return
            tags[name] = content
        embed = discord.Embed(description=f'Tag `{name}` added.', color=
            discord.Color.green())
        await ctx.send(embed=embed)
    ...
    async def remove(self, ctx: commands.Context, name: str):
        ...
        async with self.config.guild(ctx.guild).tags() as tags:
            if name not in tags:
                embed = discord.Embed(description=
                    f'The tag `{name}` does not exist.', color=discord.
                    Color.red())
                await ctx.send(embed=embed)
                return
            del tags[name]
        embed = discord.Embed(description=f'Tag `{name}` removed.', color=
            discord.Color.green())
        await ctx.send(embed=embed)
    ...
    async def list(self, ctx: commands.Context):
        ...
        if not tags:
            embed = discord.Embed(description='No tags available.', color=
                discord.Color.blue())
            await ctx.send(embed=embed)
            return
        tag_list = '\n'.join(tags.keys())
        embed = discord.Embed(description=f'Available tags:\n{tag_list}',
            color=discord.Color.blue())
        await ctx.send(embed=embed)
    ...
    async def run(self, ctx: commands.Context, tag: str):
        ...
        if tag in tags:
            ...
            embed = discord.Embed(description=tags[tag], color=color)
            await ctx.send(embed=embed)
        else:
            close_matches = get_close_matches(tag, tags.keys())
            if close_matches:
                embed = discord.Embed(description=
                    f'Tag `{tag}` not found. Did you mean `{close_matches[0]}`?'
                    , color=discord.Color.red())
                await ctx.send(embed=embed)
            else:
                embed = discord.Embed(description=
                    f'Tag `{tag}` not found and no close matches were found.',
                    color=discord.Color.red())
                await ctx.send(embed=embed)

    @run.autocomplete('tag')
    async def autocomplete_tag(self, interaction: discord.Interaction,
        current: str):
