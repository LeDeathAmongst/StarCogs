from Star_Utils import Cog, Menu, Settings, CogsUtils
...
TimeConverter: commands.converter.TimedeltaConverter = (commands.converter.
    TimedeltaConverter(minimum=None, maximum=None, allowed_units=None,
...
class DevOutput(dev_commands.DevOutput):
    ...
    def __str__(self, output_mode: typing.Literal['repr', 'repr_or_str',
        ...
        _console_custom_kwargs: typing.Dict[str, typing.Any] = self.env.get(
            '_console_custom', {'width': 80, 'no_color': True,
            'color_system': None, 'tab_size': 2, 'soft_wrap': False})
        with redirect(**_console_custom_kwargs) as console:
            with console.capture() as captured:
                if (formatted_imports := self.env.get_formatted_imports()):
                    console.print(rich.syntax.Syntax(formatted_imports,
                        'pycon', theme=SolarizedCustom))
                if self.prints:
                ...
                elif self.result is not None or self.always_include_result:
                    if output_mode == 'str':
                        result = str(self.result)
                    elif isinstance(self.result, collections.abc.Iterable
                        ) and not (output_mode == 'repr' and isinstance(
                        self.result, str)) or hasattr(self.result,
        ...
        return CogsUtils.replace_var_paths(dev_commands.sanitize_output(
            self.ctx, output)).replace('```', 'ˋˋˋ')

    async def send(self, *, tick: bool=True, output_mode: typing.Literal[
        'repr', 'repr_or_str', 'str']='repr', ansi_formatting: bool=False,
        send_interactive: bool=False, send_dpy_objects: bool=True, wait:
        bool=True) ->None:
        if send_dpy_objects and self.result is not None:
            kwargs = {}
            channel_permissions = self.ctx.channel.permissions_for(self.ctx.me)
            if isinstance(self.result, discord.Embed
                ) and channel_permissions.embed_links:
                kwargs['embed'] = self.result
            elif isinstance(self.result, discord.File
                ) and channel_permissions.attach_files:
                ...
                for element in self.result:
                    if isinstance(element, discord.Embed
                        ...
                        if len(kwargs['embeds']) < 10 and sum(len(embed) for
                            embed in kwargs['embeds']) + len(element) <= 6000:
                            kwargs['embeds'].append(element)
                    elif isinstance(element, discord.File
                        ) and channel_permissions.attach_files:
                        if sum(len(file) for file in kwargs['files']) + len(
                            element) <= 6000:
        ...
        if tick and self.exc is not None:
            await self.ctx.react_quietly(reaction='❗' if isinstance(self.
                exc, SyntaxError) else '⏰' if isinstance(self.exc, (
                TimeoutError, asyncio.TimeoutError, aiohttp.ClientTimeout,
                aiohttp.ServerTimeoutError, subprocess.TimeoutExpired)) else
                '❌')
        box_lang = ('ini' if self.ctx.command.name == 'eshell' else 'ansi' if
            ansi_formatting else 'py')
        if send_interactive:
            task = self.ctx.send_interactive([box(page, lang=box_lang) for
                page in dev_commands.get_pages((
                f"""{self.env['prefix_dev_output']}

""" if 
                'prefix_dev_output' in self.env else None) + self.__str__(
            ...
            await Menu(pages=pages, prefix=self.env.get('prefix_dev_output'
                ), lang=box_lang).start(self.ctx, wait=wait)
        if tick and self.exc is None:
            await self.ctx.react_quietly(reaction=commands.context.TICK if 
                not hasattr(commands.context, 'MORE_TICKS') else random.
                choice(list(commands.context.MORE_TICKS)))

    @classmethod
    async def from_debug(cls, ctx: commands.Context, *, source: str,
        source_cache: dev_commands.SourceCache, env: typing.Dict[str,
        typing.Any], **kwargs) ->'DevOutput':
        output = cls(ctx, source=source, source_cache=source_cache,
            filename=
            f'<debug command - snippet #{source_cache.take_next_index()}>',
            env=env, **kwargs)
        await output.run_debug()
        return output

    @classmethod
    async def from_eval(cls, ctx: commands.Context, *, source: str,
        source_cache: dev_commands.SourceCache, env: typing.Dict[str,
        typing.Any], **kwargs) ->'DevOutput':
        output = cls(ctx, source=source, source_cache=source_cache,
            filename=
            f'<eval command - snippet #{source_cache.take_next_index()}>',
            env=env, **kwargs)
        await output.run_eval()
        return output

    @classmethod
    async def from_repl(cls, ctx: commands.Context, *, source: str,
        source_cache: dev_commands.SourceCache, env: typing.Dict[str,
        typing.Any], **kwargs) ->'DevOutput':
        output = cls(ctx, source=source, source_cache=source_cache,
            filename=
            f'<repl session - snippet #{source_cache.take_next_index()}>',
            env=env, **kwargs)
    ...
    async def run_debug(self) ->None:
        ...
        _console_custom_kwargs: typing.Dict[str, typing.Any] = self.env.get(
            '_console_custom', {'width': 80, 'no_color': True,
            'color_system': None, 'tab_size': 2, 'soft_wrap': False}).copy()
    ...
    async def run_eval(self) ->None:
        ...
        try:
            parse = ast.parse('async def func():\n%s' % textwrap.indent(
                self.raw_source, '  '))
            try:
                return_found = [d for d in parse.body[0].body if isinstance
                    (d, ast.Return)][0]
            except IndexError:
            ...
            _raw_source.insert(line, textwrap.indent(
                'dev_output._locals.update(**locals())', ''))
            for line, line_text in enumerate(_raw_source.copy()):
                ...
                if _line_text.startswith('yield '):
                    _raw_source[line] = textwrap.indent(
                        f'print(repr(({_line_text[6:]})))', (len(line_text) -
                        len(_line_text)) * ' ')
            self.raw_source = '\n'.join(_raw_source)
        except SyntaxError:
            pass
    ...
    def set_exception(self, exc: Exception, *, skip_frames: int=1) ->None:
        ...
        self.formatted_exc: str = self.format_exception(exc, skip_frames=
            skip_frames)
            ...
            with console.capture() as captured:
                ...
                for _ in range(skip_frames):
                    if tb is None:
                        break
                    tb = tb.tb_next
                if issubclass(type(exc), SyntaxError
                    ) and exc.lineno is not None:
                    try:
                        source_lines, line_offset = self.source_cache[exc.
                            filename]
                    except KeyError:
                        pass
                    else:
                        if exc.text is None:
                            try:
                                ...
                                if sys.version_info >= (3, 10
                                    ) and exc.end_lineno is not None:
                            ...
                            if sys.version_info >= (3, 10
                                ...
                                exc.end_lineno -= line_offset
                rich_tb = rich.traceback.Traceback.from_exception(type(exc),
                    exc, tb, extra_lines=0, theme=SolarizedCustom)
...
class Dev(Cog, dev_commands.Dev):
    ...
    def __init__(self, bot: Red) ->None:
        ...
        self._last_locals: typing.Dict[typing.Union[discord.Member, discord
        ...
        self.config: Config = Config.get_conf(self, identifier=
            205192943327321000143939875896557571750, force_registration=True)
        self.dev_global: typing.Dict[str, typing.Union[typing.Literal[
            'repr', 'repr_or_str', 'str'], bool]] = {'auto_imports': True,
            'output_mode': 'repr', 'rich_tracebacks': False,
            'ansi_formatting': False, 'send_interactive': False,
            'send_dpy_objects': True, 'use_last_locals': True,
            'downloader_already_agreed': False, 'use_extended_environment':
            True}
        self.config.register_global(auto_imports=True, output_mode='repr',
            rich_tracebacks=False, ansi_formatting=False, send_interactive=
            False, send_dpy_objects=True, use_last_locals=True,
            downloader_already_agreed=False, use_extended_environment=True)
        _settings: typing.Dict[str, typing.Dict[str, typing.Union[typing.
            List[str], bool, str]]] = {'auto_imports': {'converter': bool,
            'description': 'Enable or disable auto imports.'},
            'output_mode': {'converter': typing.Literal['repr',
            'repr_or_str', 'str'], 'description':
    ...
    async def cog_load(self) ->None:
        ...
        if await self.config.downloader_already_agreed() and (downloader_cog :=
            self.bot.get_cog('Downloader')) is not None:
    ...
    async def cog_unload(self) ->None:
        ...
        core_dev.env_extensions: typing.Dict[str, typing.Any
            ] = self.env_extensions
    ...
    def get_environment(self, ctx: commands.Context,
        use_extended_environment: bool=True) ->DevEnv:
        return DevEnv.get_environment(ctx, use_extended_environment=
            use_extended_environment)

    async def my_exec(self, ctx: commands.Context, type: typing.Literal[
        'debug', 'eval', 'repl'], source: str, env: typing.Optional[typing.
        Dict[str, typing.Any]]=None, send_result: bool=False, wait: bool=True
        ) ->bool:
        tasks: typing.List[asyncio.Task] = [asyncio.create_task(ctx.bot.
            wait_for('message', check=MessagePredicate.cancelled(ctx))),
            asyncio.create_task(self._my_exec(ctx, type=type, source=source,
            env=env, send_result=send_result, wait=wait))]
        done, pending = await asyncio.wait(tasks, return_when=asyncio.
            FIRST_COMPLETED)
        for task in pending:
            task.cancel()
    ...
    async def _my_exec(self, ctx: commands.Context, type: typing.Literal[
        'debug', 'eval', 'repl'], source: str, env: typing.Optional[typing.
        Dict[str, typing.Any]]=None, send_result: bool=False, wait: bool=True
        ) ->DevOutput:
        source = cleanup_code(source)
        if env is None:
            env = self.get_environment(ctx, use_extended_environment=await
                self.config.use_extended_environment())
        env['auto_imports'] = await self.config.auto_imports()
        if (isinstance(ctx.author, (discord.Member, discord.User)) and ctx.
            author in self._last_locals and await self.config.use_last_locals()
            ):
            _locals = self._last_locals[ctx.author]
        else:
            _locals = {}
        types = {'debug': DevOutput.from_debug, 'eval': DevOutput.from_eval,
            'repl': DevOutput.from_repl}
        mobile = ctx.author.is_on_mobile() if isinstance(ctx.author,
            discord.Member) else False
        if await self.config.ansi_formatting():
            _console_custom_kwargs: typing.Dict[str, typing.Any] = {'width':
                37 if mobile else 80, 'no_color': mobile, 'color_system': 
                None if mobile else 'standard', 'tab_size': 2, 'soft_wrap':
                False}
        else:
            _console_custom_kwargs: typing.Dict[str, typing.Any] = {'width':
                80, 'no_color': True, 'color_system': None, 'tab_size': 2,
                'soft_wrap': False}
        ...
        output: DevOutput = await types[type](ctx, source=source,
            source_cache=self.source_cache, env=env, rich_tracebacks=await
            self.config.rich_tracebacks(), _locals=_locals)
        self._last_result = output.result
        self.dev_outputs[ctx.message] = output
        if type == 'eval' and isinstance(ctx.author, (discord.Member,
            discord.User)) and output._locals:
        ...
        if send_result:
            ...
            send_coroutine = output.send(tick=type != 'repl', output_mode=await
                self.config.output_mode(), ansi_formatting=await self.
                config.ansi_formatting(), send_interactive=send_interactive,
                send_dpy_objects=await self.config.send_dpy_objects(), wait
                =wait)
    ...
    async def _eval(self, ctx: commands.Context, *, body: str=None) ->None:
        ...
        if body is None:
            if ctx.message.attachments:
                try:
                    body = (await ctx.message.attachments[0].read()).decode(
                        encoding='utf-8')
                except UnicodeDecodeError:
                    raise commands.UserFeedbackCheckFailure(_(
                        'Unreadable attachment with `utf-8`.'))
                ...
                reference := ctx.message.reference.resolved), discord.Message):
                if (match := re.compile(
                    '(eval|ev|e|(jsk|jishaku) (py|python|eval|ev)|(runcode|executecode) (py|python))(\\n)?( )?(?P<body>(.|\\n)*)'
                    ).search(reference.content)
        ...
        await self.my_exec(getattr(ctx, 'original_context', ctx), type=
            'eval', source=source, send_result=True)
    ...
    async def repl(self, ctx: commands.Context) ->None:
        ...
        if ctx.channel.id in self.sessions:
            ...
            return
        env = self.get_environment(ctx, use_extended_environment=await self
            .config.use_extended_environment())
        env['_'] = None
        ...
        while True:
            task = asyncio.create_task(ctx.bot.wait_for('message', check=
                MessagePredicate.regex('^`', ctx)))
            ...
            try:
                output = await self._my_exec(getattr(ctx,
                    'original_context', ctx), type='repl', source=source,
                    env=env, wait=False, send_result=True)
            except Exit:
                break
            try:
                if output.formatted_exc:
                    ...
                    await response.add_reaction(commands.context.TICK if 
                        not hasattr(commands.context, 'MORE_TICKS') else
                        random.choice(list(commands.context.MORE_TICKS)))
            except discord.HTTPException:
                pass
    ...
    async def pause(self, ctx: commands.Context, toggle: bool=None) ->None:
        ...
        if toggle:
            await ctx.send(_(
                'The REPL session in this channel has been resumed.'))
        else:
    ...
    @commands.is_owner()
    @commands.hybrid_command()
    async def bypasscooldowns(self, ctx: commands.Context, toggle: typing.
        Optional[bool]=None, *, time: typing.Optional[int]=None, persist:
        bool=False) ->None:
        ...
        if toggle:
            await ctx.send(_(
                ...
                ).format(optional_duration='' if time is None else
                f' for {time} seconds'))
        else:
            await ctx.send(_(
    ...
    async def _eshell(self, ctx: commands.Context, silent: typing.Optional[
        bool]=False, *, command: str=None) ->None:
        ...
        if command is None:
            if ctx.message.attachments:
                try:
                    command = (await ctx.message.attachments[0].read()).decode(
                        encoding='utf-8')
                except UnicodeDecodeError:
                    raise commands.UserFeedbackCheckFailure(_(
                        'Unreadable attachment with `utf-8`.'))
    ...
    @configuration.command(aliases=['getenv', 'getformattedenvironment',
        'getformattedenv'])
    async def getenvironment(self, ctx: commands.Context, show_values: bool
        =True) ->None:
