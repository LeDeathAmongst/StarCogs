...
async def setup(bot: Red) ->None:
    ...
    if (core_dev := bot.get_cog('Dev')) is not None:
        if (env_extensions := getattr(core_dev, 'env_extensions', None)
            ) is not None:
            cog.env_extensions = env_extensions
        if (source_cache := getattr(core_dev, 'source_cache', None)
            ) is not None:
            cog.source_cache = source_cache
        if (dev_space := getattr(core_dev, 'dev_space', None)) is not None:
            cog.dev_space = dev_space
        if (_last_result := getattr(core_dev, '_last_result', None)
            ) is not None:
        ...
        if (sessions := getattr(core_dev, 'sessions', None)):
            ...
            raise errors.CogLoadError(f'End your REPL session{s} first: ' +
                humanize_list([('Private channel' if is_private(id) else
                f'<#{id}>') for id in sessions]))
