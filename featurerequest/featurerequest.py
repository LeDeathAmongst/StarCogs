...
class FeatureRequest(Cog):
    ...
    async def status(self, ctx: commands.Context, request_id: str):
        ...
        async with self.config.requests() as requests:
            ...
            await ctx.author.send(embed=discord.Embed(title='Feature Request Status', description=f'The request status is: **{status.capitalize()}**', color=discord.Color.blue()))
    ...
    async def status(self, ctx: commands.Context, request_id: str):
        ...
        async with self.config.requests() as requests:
            ...
            if not request_data:
                ...
                return
            status = request_data['status']
            await ctx.author.send(embed=discord.Embed(title='Slash Feature Request Status', description=f'The request status is: **{status.capitalize()}**', color=discord.Color.blue()))
