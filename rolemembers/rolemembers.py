...
class RoleMembers(Cog):
    ...
    async def rolemembers(self, ctx, *, role: str):
        ...
        if role_obj is None:
            await ctx.send(
                'Role not found. Please mention the role or provide a valid role ID.'
                )
            return
        members = [member.mention for member in role_obj.members]
        if members:
            embed = discord.Embed(title=f'Members in {role_obj.name}',
                description='\n'.join(members), color=role_obj.color)
        else:
            embed = discord.Embed(title=f'Members in {role_obj.name}',
                description='No members found in this role.', color=
                role_obj.color)
