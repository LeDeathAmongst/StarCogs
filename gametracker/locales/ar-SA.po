...
class GameTracker(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        default_guild = {'log_channel': None, 'tracked_games': [],
            'ignored_users': []}
    ...
    async def setlogchannel(self, ctx: commands.Context, channel: discord.
        TextChannel):
    ...
    async def addgame(self, ctx: commands.Context, *, game_name: str):
        ...
        async with self.config.guild(ctx.guild).tracked_games(
            ) as tracked_games:
            if game_name not in tracked_games:
                tracked_games.append(game_name)
                await ctx.send(
                    f"Game '{game_name}' added to the tracking list.")
            else:
    ...
    async def removegame(self, ctx: commands.Context, *, game_name: str):
        ...
        async with self.config.guild(ctx.guild).tracked_games(
            ...
            if game_name in tracked_games:
                ...
                await ctx.send(
                    f"Game '{game_name}' removed from the tracking list.")
            else:
    ...
    async def ignore(self, ctx: commands.Context):
        ...
        async with self.config.guild(ctx.guild).ignored_users(
            ) as ignored_users:
            if ctx.author.id in ignored_users:
                ignored_users.remove(ctx.author.id)
                await ctx.send(
                    'You have been removed from the tracking ignore list.')
            else:
                ignored_users.append(ctx.author.id)
                await ctx.send(
                    'You have been added to the tracking ignore list.')
    ...
    async def on_presence_update(self, before: discord.Member, after:
        ...
        before_games = {activity.name for activity in before.activities if
            isinstance(activity, discord.Game)}
        after_games = {activity.name for activity in after.activities if
            isinstance(activity, discord.Game)}
        started_games = after_games - before_games
        stopped_games = before_games - after_games
        for game in started_games:
            if game in tracked_games:
                embed = discord.Embed(title='Game Started', description=
                    f'{after.mention} has started playing {game}.', color=
                    discord.Color.green())
                embed.set_footer(text=f'User ID: {after.id}')
                await log_channel.send(embed=embed)
        for game in stopped_games:
            if game in tracked_games:
                embed = discord.Embed(title='Game Stopped', description=
                    f'{after.mention} has stopped playing {game}.', color=
                    discord.Color.red())
                embed.set_footer(text=f'User ID: {after.id}')
                await log_channel.send(embed=embed)
