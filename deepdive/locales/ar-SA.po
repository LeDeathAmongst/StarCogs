...
class DeepDive(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        self.config.register_global(db_path='deepdive_results.sqlite',
            other_bots=[])
    ...
    async def deepdive(self, ctx: commands.Context, username: str):
        ...
        progress_message = await ctx.send(
            f'Starting deep dive on {username}...')
        self.db_path = await self.config.db_path()
        self._setup_db()
        await progress_message.edit(content=
            f'Database setup complete for {username}. Notifying other bots...')
        ...
        await progress_message.edit(content=
            f'Deep dive in progress for {username}. Fetching results...')
        ...
        await progress_message.edit(content=
            f'Deep dive complete for {username}.')
    ...
    async def listbots(self, ctx: commands.Context):
        ...
        if not other_bots:
            ...
            embed = discord.Embed(title='Added Bots', color=39423)
            for bot in other_bots:
                embed.add_field(name=bot['name'], value=bot['token'],
                    inline=False)
            await ctx.send(embed=embed)
    ...
    def _setup_db(self):
        ...
        if not inspector.has_table('results'):
            ...
            with self.engine.connect() as conn:
                result = conn.execute(text('PRAGMA table_info(results)')
                    ).fetchall()
    ...
    async def _notify_other_bots(self, username, ctx, progress_message):
        ...
        for i, bot_info in enumerate(other_bots):
            await self._notify_bot(bot_info, username, ctx,
                progress_message, i + 1, len(other_bots))
...
**Progress:** {progress}"""
    ...
    async def _perform_local_deep_dive(self, username, ctx,
        progress_message, client=None):
        if client is None:
            client = self.bot
        ...
        for i, guild in enumerate(client.guilds, start=1):
            ...
            await progress_message.edit(content=
                f"""Deep dive in progress for {username}. Searched {i}/{total_guilds} servers...
**Progress:** {progress}"""
                )

    async def _search_guild(self, guild, username, ctx):
        members = [member async for member in guild.fetch_members(limit=None)]
        users = [member for member in members if self._is_matching_user(
            member, username)]
        if users:
    ...
    def _is_matching_user(self, member, username):
        ...
        try:
            ...
            return member.name.lower() == username.lower() or str(member
                ) == username or member.mention == username
    ...
    def _analyze_messages(self, users, messages, guild_name):
        if not messages:
            return json.dumps({'users': ', '.join([str(user) for user in
                users]), 'servers': [guild_name], 'trustworthiness':
                'Neutral', 'intent_summary': {}, 'sentiment_summary':
                'Neutral', 'top_words': '', 'avg_message_length': 0,
                'most_active_channels': {}, 'most_mentioned_users': {},
        ...
        return json.dumps({'users': ', '.join([str(user) for user in users]
            ), 'servers': [guild_name], 'trustworthiness': trustworthiness,
            'intent_summary': intent_summary, 'sentiment_summary':
            sentiment_summary, 'top_words': top_words, 'avg_message_length':
            avg_message_length, 'most_active_channels':
            most_active_channels, 'most_mentioned_users':
            most_mentioned_users, 'time_of_day_summary': time_of_day_summary})
    ...
    def _evaluate_trustworthiness(self, messages):
        ...
        positive_score = sum(1 for msg in messages if any(kw in msg.content
            .lower() for kw in positive_keywords))
        negative_score = sum(1 for msg in messages if any(kw in msg.content
            .lower() for kw in negative_keywords))
    ...
    def _analyze_sentiment(self, messages):
        sentiments = [TextBlob(msg.content).sentiment.polarity for msg in
            messages]
    ...
    def _analyze_intent(self, messages):
        intent_keywords = {'seekingHelp': ['help', 'support', 'assist',
            'issue', 'problem'], 'offeringHelp': ['assist', 'help',
            'support', 'aid', 'offer'], 'promotion': ['check out',
            'subscribe', 'follow', 'buy', 'join'], 'casual': ['hello', 'hi',
            'hey', 'lol', 'haha']}
    ...
    def _get_top_words(self, messages):
        ...
        if stop_words is None:
            stop_words = []
        filtered_messages = [msg.content for msg in messages if msg.content
            .strip() and not all(word in stop_words for word in msg.content
            .split())]
    ...
    def _aggregate_results(self, results):
        aggregated = {'users': '', 'servers': set(), 'trustworthiness':
            defaultdict(int), 'intent_summary': defaultdict(int),
            'sentiment_summary': defaultdict(int), 'top_words': [],
            'avg_message_length': 0, 'most_active_channels': defaultdict(
            int), 'most_mentioned_users': defaultdict(int),
            'time_of_day_summary': [0] * 24}
        total_messages = 0
        for result in results:
            ...
            if result_data['trustworthiness']:
                aggregated['trustworthiness'][result_data['trustworthiness']
                    ] += 1
            for intent, count in result_data['intent_summary'].items():
                aggregated['intent_summary'][intent] += count
            aggregated['sentiment_summary'][result_data['sentiment_summary']
                ] += 1
            aggregated['top_words'].extend(result_data['top_words'].split(', ')
                )
            aggregated['avg_message_length'] += result_data[
                'avg_message_length'] * len(result_data['top_words'].split(
                ', '))
        ...
        aggregated['avg_message_length'
            ] /= total_messages if total_messages else 1
        aggregated['top_words'] = ', '.join(self.tfidf_vectorizer.
    ...
    def _create_results_embeds(self, username, aggregated_results):
        embeds = []
        embed = discord.Embed(title=f'Deep Dive Results for {username}',
            color=39423)
        total_length = 0
        fields = [('Users', aggregated_results['users']), ('Servers',
            aggregated_results['servers']), ('Trustworthiness', ', '.join([
            f'{k}: {v}' for k, v in aggregated_results['trustworthiness'].
            items()])), ('Intent Summary', ', '.join([f'{k}: {v}' for k, v in
            aggregated_results['intent_summary'].items()])), (
            'Sentiment Summary', ', '.join([f'{k}: {v}' for k, v in
            aggregated_results['sentiment_summary'].items()])), (
            'Top Words', aggregated_results['top_words']), (
            'Average Message Length', str(aggregated_results[
            'avg_message_length'])), ('Most Active Channels', ', '.join([
            f'{k}: {v}' for k, v in aggregated_results[
            'most_active_channels'].items()])), ('Most Mentioned Users',
            ', '.join([f'{k}: {v}' for k, v in aggregated_results[
            'most_mentioned_users'].items()])), ('Activity by Time of Day',
            ', '.join([f'{hour}:00 - {hour + 1}:00: {count}' for hour,
            count in enumerate(aggregated_results['time_of_day_summary'])]))]
        for name, value in fields:
            ...
            if total_length + field_length > 6000:
                ...
                embed = discord.Embed(title=
                    f'Deep Dive Results for {username}', color=39423)
                total_length = 0
    ...
    async def _save_result(self, user_id, platform, result):
        ...
        new_result = Result(user_id=user_id, platform=platform, result=result)
