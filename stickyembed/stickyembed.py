...
class StickyEmbed(Cog):
    ...
    @commands.command()
    async def esticky(self, ctx, title: str, description: str, color:
        discord.Color, footer: str=None, *fields: str):
        ...
        for field in fields:
            if ':' in field:
                ...
                embed.add_field(name=name.strip(), value=value.strip(),
                    inline=False)
        if footer:
    ...
    async def on_message(self, message):
        ...
        if (self.sticky_channel and message.channel == self.sticky_channel and
            not message.author.bot and not self.stop):
            await self.sticky_message.delete()
            self.sticky_message = await self.sticky_channel.send(embed=self
                .embed)
