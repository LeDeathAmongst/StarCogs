...
class UserInstall(Cog):
    ...
    def __init__(self, bot: Red):
        ...
        self.config.register_global(client_id=None, client_secret=None,
            redirect_uri=None, bot_token=None, venv_python_path=None,
            webserver_pid=None)
    ...
    async def setvenvpythonpath(self, ctx, venv_python_path: str):
        ...
        confirmation = await ctx.send(
            'Path to the Python interpreter in the virtual environment has been set.'
            )
        await ctx.message.delete()
        await confirmation.delete()
    ...
    async def usercount(self, ctx):
        ...
        await ctx.send(
            f'The total number of users who have installed the bot is: {total_installed_users}'
            )
    ...
    async def startwebserver(self, ctx):
        ...
        if not venv_python_path:
            await ctx.send(
                'The path to the Python interpreter in the virtual environment is not set. Please set it using the command `!oauth setvenvpythonpath`.'
                )
            return
        ...
        try:
            process = subprocess.Popen([venv_python_path,
                'cogs/userinstall/webserver.py', host_ip], stdout=log_file,
                stderr=log_file)
            await self.config.webserver_pid.set(process.pid)
            await ctx.send(
                f'Web server started on {host_ip} with PID {process.pid}.')
    ...
    async def stopwebserver(self, ctx):
        ...
        try:
            ...
            await ctx.send(
                f'Web server with PID {webserver_pid} has been stopped.')
        except Exception as e:
