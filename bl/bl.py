...
class Bl(Cog):
    ...
    async def add(self, ctx, user: discord.User, link: str):
        ...
        if not any(role.id in trusted_roles for role in ctx.author.roles):
            await ctx.send(
                'You do not have permission to add to the blacklist.')
            return
        async with self.config.guild(ctx.guild).blacklist() as blacklist:
            blacklist[str(user.id)] = link
            await ctx.send(
                f'User {user} has been added to the blacklist with link: {link}'
                )
    ...
    async def remove(self, ctx, user: discord.User):
        ...
        if not any(role.id in trusted_roles for role in ctx.author.roles):
            await ctx.send(
                'You do not have permission to remove from the blacklist.')
            return
        async with self.config.guild(ctx.guild).blacklist() as blacklist:
            if str(user.id) in blacklist:
                del blacklist[str(user.id)]
                await ctx.send(
                    f'User {user} has been removed from the blacklist.')
            else:
    ...
    async def check(self, ctx, user: discord.User):
        ...
        if str(user.id) in blacklist:
            await ctx.send(
                f'User {user} is blacklisted. Link: {blacklist[str(user.id)]}')
        else:
            await ctx.send(f'User {user} is not in the blacklist.')
    ...
    async def list(self, ctx):
        ...
        if not blacklist:
            ...
            return
        embed = discord.Embed(title='Blacklisted Users', color=discord.
            Color.red())
        for user_id, link in blacklist.items():
            user = self.bot.get_user(int(user_id))
            if user:
                embed.add_field(name=str(user), value=f'[Link]({link})',
                    inline=False)
            else:
                embed.add_field(name=f'User ID: {user_id}', value=
                    f'[Link]({link})', inline=False)
